...
public class PSOL {
	...
	private double[][] bestNeighbour;
	private double[] bestNeighbourFunctionValue;
	...
	public PSOL(FFANN ffann, int populationSize, double c1, double c2, double minSpeed, double maxSpeed,
			int neigbourSize, int maxIter, double merr) {
		this.ffann = ffann;
	...
	public double[] run() {
		...
		while (iterCount < maxIter && globalBestFunctionValue > merr) {
			...
			for (int i = 0; i < population.length; i++) {
				for (int j = -neigbourSize; j < (neigbourSize + 1); j++) {
					int neighbourIndex = i + j;
					if (neighbourIndex >= population.length) {
						neighbourIndex -= population.length;
					}
					if (neighbourIndex < 0) {
						neighbourIndex += population.length;
					}
					if (functionValues[neighbourIndex] < bestNeighbourFunctionValue[i]) {
						bestNeighbour[i] = population[neighbourIndex];
						bestNeighbourFunctionValue[i] = functionValues[neighbourIndex];
					}
				}
			...
			for (int i = 0; i < population.length; i++) {
				for (int j = 0; j < weightsCount; j++) {
					speeds[i][j] = speeds[i][j] * inertion
							...
							+ c2 * Math.random() * (bestNeighbour[i][j] - population[i][j]);
	...
	private void initalizePopulation() {
		...
		bestNeighbour = new double[populationSize][weightsCount];
		bestNeighbourFunctionValue = new double[populationSize];
		...
		for (int i = 0; i < populationSize; i++) {
			...
			bestNeighbour[i] = Arrays.copyOf(neuron, neuron.length);
			bestNeighbourFunctionValue[i] = ffann.errorFunction(neuron);
